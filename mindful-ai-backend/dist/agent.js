                ...
                if (event.transcript && userId) {
                    ...
                    if (userInput.length > 3 && !userInput.toLowerCase().includes('hello') && !userInput.toLowerCase().includes('hi')) {
                        try {
                            ...
                            const { rows } = await pool.query('SELECT session_transcript, session_summary FROM user_sessions WHERE user_id = $1 AND embedding IS NOT NULL ORDER BY embedding <-> $2 LIMIT 3', [userId, `[${inputEmbedding.join(',')}]`]);
                                    ...
                                    if (row.session_transcript) {
                                        try {
                                            ...
                                            if (Array.isArray(transcriptArray)) {
                                                transcript = transcriptArray.map((item) => {
                                                    try {
...
Please use this context to provide more personalized and relevant responses.`;
            ...
            session.on('input_speech_committed', (event) => {
                console.log('=== INPUT SPEECH COMMITTED ===', event);
            });
            session.on('response_content_added', (event) => {
                console.log('=== RESPONSE CONTENT ADDED ===', event);
            });
            session.on('response_done', (event) => {
