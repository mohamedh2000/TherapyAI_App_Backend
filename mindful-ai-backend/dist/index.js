...
app.post('/api/connection-details', async (req, res) => {
    try {
        ...
        // Fetch embeddings
        const embeddingsResult = await pool.query('SELECT embedding FROM user_sessions WHERE user_id = $1', [userId]);
        ...
        const roomName = `voice_assistant_room_${userId}_${Math.floor(Math.random() * 10000)}`;
...
app.post('/api/save-transcript', async (req, res) => {
    ...
    try {
        const result = await pool.query('INSERT INTO user_sessions (user_id, session_date, total_time_minutes, session_rating, session_summary, session_transcript, embedding) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *', [userId, new Date(), sessionDuration, sentiment, subjects, transcript, embeddingString]);
